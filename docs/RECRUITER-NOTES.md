...
Concrete talking points mapped to where in the code each is demonstrable. Read alongside the [DEMO.md](../DEMO.md) screen-share runbook. The goal is to be specific: not "the parser handles realtime data" but "[gtfs_dleung/parser/tripupdates.py](../gtfs_dleung/parser/tripupdates.py) line 143 is where partial-update propagation lives."
...
**Where**: [gtfs_dleung/parser/tripupdates.py:140-175](../gtfs_dleung/parser/tripupdates.py#L140-L175), function `_arrivals_for_scheduled_trip`.
...
**Outbound** (us-as-consumer): [gtfs_dleung/fetcher/rate_limit.py:19-67](../gtfs_dleung/fetcher/rate_limit.py#L19-L67) — `OutboundRateLimiter`. Per-URL minimum-interval enforcement; ≤ 1 fetch / 10s per feed by default. Protects MBTA's CDN from us.

**Inbound** (us-as-server): [gtfs_dleung/security/rate_limit.py:41-104](../gtfs_dleung/security/rate_limit.py#L41-L104) — `SessionRateLimiter`. Sliding-window per Streamlit session; default 30 req / 60s. Protects us from a runaway authenticated session.
...
**Where**: [gtfs_dleung/fetcher/health.py:87-132](../gtfs_dleung/fetcher/health.py#L87-L132), `HealthTrackedFetcher.fetch`.

Each tier kicks in when the one above fails:

1. **Live fetch** — happy path, fresh bytes from MBTA's CDN ([fetcher/realtime.py](../gtfs_dleung/fetcher/realtime.py)).
2. **Soft cache** — `dict[str, FeedMessage]` of last-success messages, in memory. Survives transient network blips within a process.
3. **Hard snapshot** — committed `.pb` files under [examples/](../examples/), loaded by [fetcher/fallback.py](../gtfs_dleung/fetcher/fallback.py). Stale by design but keeps the UI usable through a cold-start outage.

The `FeedHealth` model exposes two independent flags — `is_stale` (the **data** is old) and `is_degraded` (our **fetch** is failing). Together they cover the four combinations operators actually care about. The `is_snapshot` flag (new in [#13](https://github.com/dcltdw/gtfs-dleung/issues/13)) surfaces the third tier so the UI labels honestly.

### 4. Auth surface that never logs passwords

**Where**: [gtfs_dleung/auth.py:40-79](../gtfs_dleung/auth.py#L40-L79) — `verify_credentials`. Bcrypt-checked; emits structured `auth.login.success` / `auth.login.failure` records with three distinct failure reasons (`unknown_user` / `wrong_password` / `invalid_hash`).
...
The Streamlit-page wiring sits one layer up ([gtfs_dleung/app.py](../gtfs_dleung/app.py)), using `build_authenticator_config` to feed the bcrypt hash + cookie key into `streamlit-authenticator`. The cookie HMAC key is a **separate setting** from the password hash so "hash leaked" and "sessions forgeable" stay distinct threats.
...
Why mention this: the *what* of the project is GTFS-RT competence; the *how* is process. Both matter for a real team. The PR audit trail on the repo's [merged-PR list](https://github.com/dcltdw/gtfs-dleung/pulls?q=is%3Apr+is%3Aclosed+is%3Amerged) is the artefact.
...
- **MBTA-specific.** The parsers would run unchanged against any GTFS-RT publisher, but the corridor + scope decisions ([gtfs_dleung/scope.py](../gtfs_dleung/scope.py)) are MBTA-specific.
