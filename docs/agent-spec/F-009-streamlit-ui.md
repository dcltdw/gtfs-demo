...
3. **Two-column arrivals board.** `next_n_arrivals` (F-003) picks five upcoming arrivals at `place-davis` and `place-balsq`. Each row shows the route, scheduled time, predicted time, and a color-coded delay magnitude (`green` ≤ 30 s, `yellow` ≤ 120 s, `red` > 120 s, `neutral` if unknown). Unusual `schedule_relationship` values render badges — `CANCELED` strikes through, `ADDED` is purple, `SKIPPED`/`UNSCHEDULED` are gray.
