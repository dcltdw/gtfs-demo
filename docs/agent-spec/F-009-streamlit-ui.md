...
3. **Two-column arrivals board, each station split by direction.** `next_n_arrivals` (F-003) picks five upcoming arrivals at `place-davis` and `place-balsq`. Each station column renders **two subsections** — Inbound (toward downtown) first, then Outbound (away from downtown) — each subsection showing its own top-5. Per-row, the renderer shows route, scheduled time, predicted time, color-coded delay magnitude (`green` ≤ 30 s, `yellow`/orange ≤ 120 s, `red` > 120 s, `neutral` if unknown) — **both the leading dot emoji and the delay text** are wrapped in the same color (`:green[…]` / `:orange[…]` / `:red[…]`) so the visual signal isn't reliant on a single dot — and the trip's headsign (e.g. `toward Alewife`). Unusual `schedule_relationship` values render badges — `CANCELED` strikes through, `ADDED` is purple, `SKIPPED`/`UNSCHEDULED` are gray.
...
6. **Auto-refresh every 15 s.** `streamlit_autorefresh.st_autorefresh(interval=15_000)` triggers a rerun. The fetcher path is gated by the inbound RL so a single session can't dominate.
...
9. **Pure helpers stay pure.** Everything except `gtfs_dleung.app` itself is unit-testable without Streamlit. `gtfs_dleung.presenter.formatters` carries the per-row display logic; the corresponding test file (`tests/test_formatters.py`, 41 tests) is the bulk of this PR's verification.
...
- `gtfs_dleung/app.py` — Streamlit entrypoint. Run with `uv run streamlit run gtfs_dleung/app.py` (or `just demo`).
- `gtfs_dleung/presenter/formatters.py` — pure display helpers (`format_arrival_row`, `format_alert_row`, `format_feed_age`, `delay_color_class`, `schedule_relationship_badge`, `feed_health_icon`, `should_show_stale_banner`).
...
- `tests/test_app_smoke.py::test_streamlit_app_starts_cleanly` — `@pytest.mark.live`. Spawns `streamlit run gtfs_dleung/app.py --server.headless=true` for ~8 s; asserts no startup crash by checking for `Traceback` in captured output.
