...
- **Feed staleness vs fetch degradation**: two independent signals on `FeedHealth`. **Stale** means the data is old — `now − header.timestamp` exceeds a **per-feed** threshold: `GTFS_TRIP_UPDATES_STALE_S` / `GTFS_VEHICLE_POSITIONS_STALE_S` (default 30s, since MBTA publishes those every ~5s) and `GTFS_SERVICE_ALERTS_STALE_S` (default 300s = 5 min, since MBTA only rebuilds the Alerts feed when an alert changes — its timestamp is routinely tens of minutes old by design). **Degraded** means our most recent fetch failed and we're serving the cached last-good message. Fresh data over a now-broken connection is `is_stale=False, is_degraded=True`; perpetually-stale publisher with our fetches still succeeding is `is_stale=True, is_degraded=False`.
