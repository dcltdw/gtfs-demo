...
This is the narrative specification for the gtfs-demo spike. Functional requirements (F-NNN) and non-functional requirements (NF-NNN) have structured counterparts under [docs/agent-spec/](docs/agent-spec/); the contract for those files is in [docs/agent-spec/schema.md](docs/agent-spec/schema.md).
...
The locked decisions in [project memory](https://github.com/dcltdw/gtfs-demo/issues/1) capture the immovable choices. The summary below restates the contract the code must satisfy. Three load-bearing decisions get their own ADRs under [docs/adr/](docs/adr/):
...
- **F-005 ServiceAlerts**: parse + scope-filter alerts; surface only those whose `informed_entity` touches Red Line / Green-E / a corridor parent station, and whose `active_period` overlaps `now`. See [docs/agent-spec/F-005-servicealerts.md](docs/agent-spec/F-005-servicealerts.md).
